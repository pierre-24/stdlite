...
void test_property_first_hyperpolarizability_TD_SOS_ok() {
    ...
    float* t0mdipstd = malloc(ctx->ncsfs * 6 * sizeof(float ));
    ASSERT_STDL_OK(stdl_property_tensor_g2e_moments(ctx, (stdl_operator[]) {STDL_OP_DIPL, STDL_OP_DIPL}, (double*[]){dipoles_mat, dipoles_mat}, ctx->ncsfs, Xamptd, Yamptd, t0mdipstd));
...
void test_property_first_hyperpolarizability_TDA_SOS_ok() {
    ...
    // get 0→m transition dipoles
    float* t0mdipstda = malloc(ctx->ncsfs * 6 * sizeof(float ));
    ASSERT_STDL_OK(stdl_property_tensor_g2e_moments(ctx, (stdl_operator[]) {STDL_OP_DIPL, STDL_OP_DIPL}, (double*[]){dipoles_mat, dipoles_mat}, ctx->ncsfs, Xamptda, NULL, t0mdipstda));
