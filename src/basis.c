...
int stdl_basis_dsp_dipvel(stdl_basis *bs, double *dipvels) {
    assert(bs != NULL && dipvels != NULL);

    stdl_log_msg(0, "Computing <µ|∇|ν> elements >");
    ...
    for(int ibas=0; ibas < bs->nbas; ibas++) {
        ...
        for(int jbas=0; jbas <= ibas; jbas++) {
            if(bs->use_spherical) {
                sj = CINTcgto_spheric(jbas, bs->bas);
                int1e_p_sph(buff, NULL, (int[]) {ibas, jbas}, bs->atm, bs->natm, bs->bas, bs->nbas, bs->env, NULL, NULL);
            }
            else {
                sj = CINTcgtos_cart(jbas, bs->bas);
                int1e_p_cart(buff, NULL, (int[]) {ibas, jbas}, bs->atm, bs->natm, bs->bas, bs->nbas, bs->env, NULL, NULL);
            }

            for(int iprim=0; iprim < si; iprim++) {
                for(int jprim=0; jprim < sj && joffset + jprim <= ioffset + iprim; jprim++) {
                    dipvels[0 * ndips + STDL_MATRIX_SP_IDX(ioffset + iprim, joffset + jprim)] = buff[0 * si * sj + jprim * si + iprim] * renorm[ioffset + iprim] * renorm[joffset + jprim];
                    dipvels[1 * ndips + STDL_MATRIX_SP_IDX(ioffset + iprim, joffset + jprim)] = buff[1 * si * sj + jprim * si + iprim] * renorm[ioffset + iprim] * renorm[joffset + jprim];
                    dipvels[2 * ndips + STDL_MATRIX_SP_IDX(ioffset + iprim, joffset + jprim)] = buff[2 * si * sj + jprim * si + iprim] * renorm[ioffset + iprim] * renorm[joffset + jprim];
                }
            }
